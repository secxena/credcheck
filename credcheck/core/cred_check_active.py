...
class dynamicTest(object):
    ...
    def checkIt(self,target,input_data):
        """
        ...
        if config_block:
            ...
            if config.get('basic'):
                if len(config.get('args'))>1:
                    respo =CheckAPI(verify_ssl=False,
                                    username=config['basic']['username'],
                                    password=config['basic']['password'])._auth(config['url'],method, **config['args'])
                else:
                    respo =CheckAPI(verify_ssl=False,
                                    username=config['basic']['username'],
                                    password=config['basic']['password'])._auth(config['url'],method)
            if config.get('args'):
                respo = CheckAPI(verify_ssl=False)._auth(config['url'],
                                                    method,**config['args'])
            else:
                respo = CheckAPI(verify_ssl=False)._auth(config['url'],  
                                                                    method)
